...
<main>
  ...
  <div class="two-col">
    <div class="chart-card">
      ...
      <div class="chart-sub">Comparativo entre unidades — ordenado do maior para o menor</div>
...
<script>
  ...
  // Barras horizontais de matrículas — ordenado maior → menor
  ...
  new Chart(document.getElementById('donutChart'), {{
    ...
    options: {{
      ...
      plugins: {{
        ...
        tooltip: {{
          backgroundColor: '#1a2340', borderColor: '#d0d8ea', borderWidth: 1,
          titleColor: '#ffffff', bodyColor: '#d0d8ea',
          callbacks: {{
            label: ctx => ` ${{ctx.raw}} matrículas — ${{((ctx.raw / {total_matr}) * 100).toFixed(1)}}% do total`
          }}
        }}
      }},
      scales: {{
        x: {{
          grid: {{ color: 'rgba(208,216,234,.3)' }},
          ticks: {{ font: {{ size: 10 }}, color: '#5c6b8a' }}
        }},
        y: {{
          grid: {{ display: false }},
          ticks: {{ font: {{ size: 11, weight: '500' }}, color: '#1a2340' }}
        }}
      }}
    }}
  }});
